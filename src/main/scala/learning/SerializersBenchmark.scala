...
import learning.Datamodel.ProtobufVote
...
object VoteGenerator {
  ...
  def generateProtobufVotes(count: Int): immutable.Seq[ProtobufVote] = (1 to count).map { _ =>
...
object VotingStation extends App {
 ...
 // val votes: immutable.Seq[Vote] = VoteGenerator.generateVotes(100000)
  val votes: immutable.Seq[ProtobufVote] = VoteGenerator.generateProtobufVotes(1000000)
...
object SerializersBenchmark_Persistence extends App {
  ...
  //val simplePersistentActor = system.actorOf(SimplePersistentActor.props("benchmark-java", false), "benchmark")
 // val simplePersistentActor = system.actorOf(SimplePersistentActor.props("benchmark-avro", false), "benchmark")
 // val simplePersistentActor = system.actorOf(SimplePersistentActor.props("benchmark-kryo", false), "benchmark")
 ...
 val votes = VoteGenerator.generateProtobufVotes(10000)
  ...
  /**
    ...
    *  108.2505000000000000 | benchmark-kryo //wins, twice memory efficient than java
    */
}